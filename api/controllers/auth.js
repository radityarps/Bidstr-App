...
const verifyOTP = async (req, res, email) => {
    ...
    try {
      ...
      const data =  await db.query(`SELECT * FROM users WHERE user_id = $1`, [email])
      const user = data.rows[0];
      if (!user) {
        res.status(401).json({message : 'Invalid email or password'})
      }
...
const register = async ( req, res) => {
  const db = await pool.connect();
    try {
      ...
      const isUserExisted =  await db.query(`SELECT * FROM users WHERE username = $1`, [username])
      const isEmailExisted =  await db.query(`SELECT * FROM users WHERE email = $1`, [email])
        ...
        if (!username || !email || !password) {
            ...
            if (!username.length > 5 && !username <= 20){
              ...
              if (!re.test(email)) {
                ...
                // password validation
                if (!password >= 8 ) {
                  return res.status(400).json({ message: 'Password minimal have 8 characters' });
                }
                  if (password !== confirmPassword) {
                      return res.status(400).json({message : "Password did not match!"});
                        ...
                        res.status(200).json({message: `Register Sucsess ${otp}`,
